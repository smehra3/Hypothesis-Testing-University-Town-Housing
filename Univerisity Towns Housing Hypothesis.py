...
def get_list_of_university_towns():
    ...
    with open('university_towns.txt') as f:
        utowns_txt = f.readlines()
    
    for line in utowns_txt:
        RegionName=line[:-1]
        if '[edit]' in line:
            state=line[:line.index('[')]
            continue
        elif '(' in line:
            RegionName=line[:line.index('(')]
        data.append((state,RegionName.strip()))
...
def get_recession_start():
    ...
    for i in range(len(dt) - 2):
        if (dt.iloc[i][1] > dt.iloc[i+1][1]) & (dt.iloc[i+1][1] > dt.iloc[i+2][1]):
            quarters.append(dt.iloc[i][0])
    return quarters[0]
...
def get_recession_end():
    ...
    dt = dt.iloc[start_index:]
    for i in range(2, len(dt)):
        if (dt.iloc[i-2][1] < dt.iloc[i-1][1]) and (dt.iloc[i-1][1] < dt.iloc[i][1]):
            return dt.iloc[i][0]
